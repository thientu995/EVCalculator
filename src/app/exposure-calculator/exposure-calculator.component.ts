...
import { ExposureCalculatorModule } from './exposure-calculator.module';
...
@Component({
  selector: 'app-exposure-calculator',
  templateUrl: './exposure-calculator.component.html',
  styleUrls: ['./exposure-calculator.component.css']
})
export class ExposureCalculatorComponent implements OnInit {
  Math = Math;
  title = 'Exposure Calculator';

  constructor(library: FaIconLibrary
    , private httpService: HttpService
    , private ecModule: ExposureCalculatorModule
  ...
  onCal() {
    ...
    this.valueResult.EV = this.ecModule.calEV(this.valueBasic.Aperture, sB, 100);
  ...
  startCountdown(value: number) {
    ...
    var msToTime = (s) => {
      ...
      return this.ecModule.formatPad(hrs)
        + ':' + this.ecModule.formatPad(mins)
        + ':' + this.ecModule.formatPad(secs)
        + '.' + this.ecModule.formatPad(ms);
    }
